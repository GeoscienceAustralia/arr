...
<xsl:stylesheet exclude-result-prefixes="d" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
   ...
   xmlns:axf="http://www.antennahouse.com/names/XSL/Extensions" version="1.0">

   <!-- Override the d:set to insert our PDF cover and back pages -->
   <xsl:template match="d:set">
      ...
      <!-- force no preamble or additional content
      <xsl:variable name="preamble" select="*[not(self::d:book or self::d:set or self::d:setindex)]"/>
      -->
      ...
      <xsl:if test="$preamble">
         <fo:page-sequence hyphenate="{$hyphenate}" master-reference="{$titlepage-master-reference}">
      ...
      <xsl:if test="contains($toc.params, 'toc')">
         <fo:page-sequence hyphenate="{$hyphenate}" master-reference="{$lot-master-reference}">
            <xsl:attribute name="language">
               <xsl:call-template name="l10n.language"/>
            </xsl:attribute>
            <xsl:attribute name="format">
   ...
   <xsl:template name="arr.set.front.cover">
      <xsl:variable name="master-reference">titlepage</xsl:variable>
      <xsl:variable name="gentext-key" select="local-name(.)"/>
      ...
      <fo:page-sequence hyphenate="{$hyphenate}"
         ...
         <fo:flow flow-name="xsl-region-body">
            ...
            <fo:block-container absolute-position="absolute" width="{$page.width}"
               height="{$page.height}" top="-(1mm + {$page.margin.top} + {$body.margin.top})"
            ...
            <fo:block-container absolute-position="absolute" width="{$page.width}" height="{$page.height}"
               ...
               <fo:block text-align="center">
                  <fo:external-graphic src="figures/base_layout_for_pdf_Part1.pdf"/>
               </fo:block>
            </fo:block-container>
   ...
   <xsl:template name="header.table">
      ...
      <xsl:choose>
         <xsl:when
            test="
               $pageclass = 'titlepage' and ($gentext-key = 'book' or $gentext-key = 'set')
               and $sequence = 'first'">
   ...
   <xsl:template name="set.titlepage.recto">
      <!-- override to insert nothing here -->
   </xsl:template>
</xsl:stylesheet>
